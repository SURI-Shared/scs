...
*/
  __global__ void _cuda_accum_by_a(scs_int n, float *y, const scs_int *Ap, const scs_int *Ai, 
    ...
    // scs_int p = blockIdx.y*blockDim.y + threadIdx.y + Ap[j];
    // scs_int i = Ai[p];
    if (j < n) {
      int start = Ap[j];
      int end = Ap[j + 1];
  ...
  void SCS(accum_by_a)(const ScsMatrix *A, const scs_float *x,  scs_float *y) {
    ...
    cudaMemcpy(d_Ai, A->i, n * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Ax, A->x, n * sizeof(scs_float), cudaMemcpyHostToDevice);
    ...
    cudaMalloc(&d_y,m *sizeof(scs_float));
    cudaMemcpy(d_y, y, m*sizeof(scs_float),cudaMemcpyHostToDevice);
    _cuda_accum_by_a<<<rows, 256>>>(n, d_y, d_Ap, d_Ai, d_Ax, d_x);
    cudaMemcpy(y, d_y, m*sizeof(scs_float),cudaMemcpyDeviceToHost);
    cudaFree(d_Ap);
    cudaFree(d_Ai);
    cudaFree(d_Ax);
    cudaFree(d_x);
 ...
 */
  ...
  cudaMalloc(&d_y,m *sizeof(scs_float));
  cudaMemcpy(d_y, y, m*sizeof(scs_float),cudaMemcpyHostToDevice);
  _cuda_accum_by_atrans<<<rows, 256>>>(n, d_y, d_Ap, d_Ai, d_Ax, d_x);
  cudaMemcpy(y,d_y,m*sizeof(scs_float),cudaMemcpyDeviceToHost);
  cudaFree(d_Ap);
  cudaFree(d_Ai);
  cudaFree(d_Ax);
  ...
  void SCS(accum_by_p)(const ScsMatrix *P, const scs_float *x, scs_float *y) {
    ...
    cudaMalloc(&d_y,m *sizeof(scs_float));
    cudaMemcpy(d_y, y,m*sizeof(scs_float),cudaMemcpyHostToDevice);
    _cuda_accum_by_p<<<1, dim3(rows, n)>>>(n, d_y, d_Pp, d_Pi, d_Px, d_x);
    cudaMemcpy(y, d_y,m*sizeof(scs_float),cudaMemcpyDeviceToHost);
    cudaFree(d_Pp);
    cudaFree(d_Pi);
    cudaFree(d_Px);
